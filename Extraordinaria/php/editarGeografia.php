...
class editarGastronomia{
    ...
    $consultaPre = $this->db->prepare("SELECT * FROM geografia WHERE nombre = ?");
    ...
    if ($resultado->fetch_assoc()!=NULL) {
        ...
        $consultaPre = $this->db->prepare("UPDATE geografia SET tipo = ?, descripcion = ?, concejo = ? WHERE nombre = ?");
        $consultaPre->bind_param('s', $_POST["tipo"]);
